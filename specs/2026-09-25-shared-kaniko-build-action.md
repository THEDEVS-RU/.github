...
- `image`: обязательный, имя целевого образа (например, `ghcr.io/thedevs-ru/thedevslk`).
- `tag`: необязательный, по умолчанию пустая строка, тег образа (если передан, объединяется с `image` как `${IMAGE}:${TAG}`).
